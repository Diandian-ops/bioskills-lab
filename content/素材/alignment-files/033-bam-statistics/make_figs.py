#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""033 bam-statistics 出图：基于 033 fig_data.json 真实平均覆盖。"""
import os, json
import matplotlib
matplotlib.use("Agg")
...
contigs = D["contigs"]; cov = D["mean_coverage"]
fig, ax = plt.subplots(figsize=(8.6, 5.0), dpi=150)
ax.bar(contigs, cov, color=CELADON, width=0.55, zorder=3)
ax.set_ylabel("mean coverage depth (x)", fontsize=10.5)
ax.set_title("Mean coverage per contig (samtools coverage)", fontsize=12, pad=12)
ax.set_ylim(0, max(cov) * 1.20)
for c, v in zip(contigs, cov):
    ax.text(c, v + max(cov) * 0.015, "%.0f" % v, ha="center", va="bottom", fontsize=9.5, color="#222222")
ax.grid(axis="y", alpha=0.25, ls=":", lw=0.8, zorder=0)
footnote(fig, "samtools coverage / samtools stats SN / samtools bedcov all derive from the same per-base pileup")
f1 = savefig(fig, "fig1_mean_coverage.png")
