#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""034 pileup-generation 出图：基于 034 fig_data.json 真实深度分布。"""
import os, json
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt

BASE = os.path.dirname(os.path.abspath(__file__))
D = json.load(open(os.path.join(BASE, "fig_data.json")))
BRICK = "#b5482f"; CELADON = "#2f7d72"; GREY = "#6b7280"

def _verify(fig, name):
    fig.canvas.draw(); r = fig.canvas.get_renderer(); fails = 0
    for ax in fig.axes:
        axb = ax.get_window_extent(r); bbs = []
        for t in ax.texts:
            bb = t.get_window_extent(r)
            if (bb.x0 < axb.x0 - 3 or bb.x1 > axb.x1 + 3 or bb.y0 < axb.y0 - 3 or bb.y1 > axb.y1 + 3):
                print("  [FAIL] %s: %r" % (name, t.get_text()[:50])); fails += 1
            bbs.append(bb)
        for i in range(len(bbs)):
            for j in range(i + 1, len(bbs)):
                a, b = bbs[i], bbs[j]
                if not (a.x1 + 1 < b.x0 or b.x1 + 1 < a.x0 or a.y1 + 1 < b.y0 or b.y1 + 1 < a.y0):
                    print("  [FAIL] %s: overlap" % name); fails += 1
    if fails == 0: print("  [PASS] %s" % name)
    return fails

def savefig(fig, name):
    fails = _verify(fig, name)
    fig.savefig(os.path.join(BASE, name), dpi=150); plt.close(fig); return fails

def footnote(fig, text):
    fig.text(0.5, 0.012, text, ha="center", va="bottom", fontsize=8.5, color=GREY)

hist = {int(k): v for k, v in D["depth_hist"].items()}
depths = sorted(hist.keys())
counts = [hist[d] for d in depths]
fig, ax = plt.subplots(figsize=(8.6, 5.0), dpi=150)
ax.bar([str(d) for d in depths], counts, color=[BRICK if d == 0 else CELADON for d in depths], width=0.9, zorder=3)
ax.set_xlabel("per-position depth (from text pileup)", fontsize=10.5)
ax.set_ylabel("number of positions", fontsize=10.5)
ax.set_title("Per-position depth distribution  (11962 positions, mean 25.4x, max 54x)", fontsize=11.5, pad=12)
ax.set_ylim(0, max(counts) * 1.18)
ax.grid(axis="y", alpha=0.25, ls=":", lw=0.8, zorder=0)
footnote(fig, "samtools mpileup now emits TEXT pileup only; BCF/VCF needs bcftools mpileup (not installed here)")
f1 = savefig(fig, "fig1_depth_distribution.png")

total = f1
print("FIGURE QUALITY: TOTAL FAILS = %d" % total)
import sys; sys.exit(1 if total else 0)
