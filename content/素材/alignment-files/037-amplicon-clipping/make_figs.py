#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""037 amplicon-clipping 出图：基于 037 fig_data.json 真实 soft-clip 计数。"""
import os, json
import matplotlib
matplotlib.use("Agg")
...
before = D["before_softclip"]; after = D["after_softclip"]
fig, ax = plt.subplots(figsize=(8.6, 4.8), dpi=150)
ax.bar(["before ampliconclip", "after ampliconclip"], [before, after], color=[GREY, BRICK], width=0.5, zorder=3)
ax.set_ylabel("reads with soft-clip (CIGAR S)", fontsize=10.5)
ax.set_title("samtools ampliconclip: soft-clipped reads 0 -> 53", fontsize=12, pad=12)
ax.set_ylim(0, max(before, after) * 1.5 + 3)
for i, v in enumerate([before, after]):
    ax.text(i, v + 0.6, str(v), ha="center", va="bottom", fontsize=10, color="#222222")
ax.grid(axis="y", alpha=0.25, ls=":", lw=0.8, zorder=0)
footnote(fig, "Primer BED defines amplicon boundaries; bases under primers become soft-clips (S), excluded from variant calling")
f1 = savefig(fig, "fig1_softclip_counts.png")
