...
x = range(len(cats))
w = 0.36
b1 = ax.bar([i - w/2 for i in x], default, w, label='PairwiseAligner() default (gap=0)', color='#c0392b')
b2 = ax.bar([i + w/2 for i in x], recommended, w, label='Recommended open=-11 / extend=-1', color='#2c7a4b')
...
labels = ['PID1\n(gap-aware)', 'PID2\n(non-gap pairs)', 'PID3\n(shorter len)', 'PID4\n(mean len)']
bars = ax.bar(labels, pids, color=['#2c7a4b','#c0392b','#2c7a4b','#2c7a4b'])
ax.set_ylim(40, 50)
