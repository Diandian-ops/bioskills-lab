...
- 现状基线：004–027 全部 `ERROR=0 WARN=0`（2026-08-29 复核）；002 的 METAPHOR 是早期遗留（001–003 保持扁平不动，不改）。改完工具或新增笔记后跑一次全量回归确认无新增 ERROR。
- **表头不算上下文**：给表格补单位时要写进**单元格**（如 `1409 bp`），写在表头（如 `END-POS（个碱基）`）仍会判 BARE_NUM；标题里孤立的编号（如 `018`）同样会触发，去掉即可（编号由文件名承载）。

## 素材可复现性 + pipeline 忽略规则（2026-08-29 收尾确立）
- **`.gitignore` 只忽略 015 及之后的工作区**：`pipeline/01[5-9]-*/` 与 `pipeline/02[0-9]-*/`。**004–006 早期工作区已入库，绝不能 blanket 写成 `pipeline/0*/`**。
- **素材脚本禁止引用 `pipeline/`**：`pipeline/` 不入库，一旦 `make_figs.py` 用 `../../../../pipeline/00X-*` 取数，clone 后必然跑不起来（017/019 曾如此，已修）。输入数据一律**复制进素材目录**，脚本内 `HERE` 自引用。
- **验收动作**：从 `/tmp` 逐个跑 `python <素材>/make_figs.py`，9/9 通过才算自包含。
- **VCF 解析统一用 `bcftools query`，不用 cyvcf2**：cyvcf2 在受管 venv 重建后已丢失，016 素材脚本因此长期不可复现（已改为 bcftools）。其余 8 个素材脚本本就用 bcftools。

## 出图高频 FAIL：轴外底部注释（017/018/019/020 连续 4 次）
- `ax.text(0.5, -0.22, ..., transform=ax.transAxes)` 这类「放轴下方当脚注」的写法**必被判文本越界**。
- 正确做法二选一：① 把说明并入 `set_title()` 的双行标题（标题区不参与重叠检测，信息还更归一）；② 放图内空白区（axes 坐标 y≈0.05）。
- 顶部同理：条形/点图最高元素的标签会被上边界裁掉，须给 y 轴留上方余量（024 曾踩）。
